...
from apps.classif_ai.models import File, FileSet, UploadSession, GTClassification


class GTClassificationTest(ClassifAiTestCase, ValidationErrorTestMixin, IntegrityErrorTestMixin):
    @classmethod
    def setUpTestData(cls):
        super(GTClassificationTest, cls).setUpTestData()
        ...
        GTClassification.objects.create(file=cls.file)

    def setUp(self) -> None:
        super(GTClassificationTest, self).setUp()
        self.file = self.__class__.file

    def test_field_validation(self):
        gt_classification = GTClassification(file=None)
        with self.assertValidationErrors(["file"]):
            gt_classification.full_clean()

        gt_classification = GTClassification(file_id=0)
        with self.assertValidationErrors(["file"]):
            gt_classification.full_clean()

    def test_unique_together(self):
        gt_classification = GTClassification(file=self.file)
